...
def display_user_meal_evaluation(request):
    ...
    try:
        ...
        template_data = {'diet_rating': diet_rating}
...
def get_user_meal(user_uid, meal_time):
    ...
    print("Meal Nutrient: ", meal_nutrient)
...
def save_user_evaluation(user_uid, meal_time, diet_rating, user_meal_nut):
    new_user_evaluation_data = Usermealevaluation(
        user_id=user_uid, meal_date=meal_time, sum_carb=user_meal_nut[0], sum_protein=user_meal_nut[1],
        sun_fat=user_meal_nut[2], meal_evaluation=diet_rating
    )
    new_user_evaluation_data.save()
...
#     def display_user_meal_evaluation(self, request):
#         print(request, "@@@@@@@@@@@@@@@@@@@@@@@")
...
#             user_id = str(user_info.get('uuid', '')) # 실제로는 이렇게 사용자 식별자를 추출하는 코드를 추가해야 합니다.
#             print(user_id)
#             meal_time = request.query_params.get('meal_time', '2023-12-29')
            
#             # 데이터베이스에서 해당 user_uid에 해당하는 객체 가져오기
#             user_object = Usermeal.objects.get(user_id=user_id)
#             print(user_id, "@@@@@@@@@@@@@")
#             diet_rating = self.evaluate_user_meal(token, meal_time)
#             template_data = {'diet_rating': diet_rating}
#             return render(request, 'user_meal_evaluation.html', template_data)
...
#     def get_user_meal(self, user_uid, meal_time):
#         user_meals = Usermeal.objects.filter(user_id=user_uid, meal_date=meal_time).values(
#             'food_name__carbs_g', 'food_name__protein_g', 'food_name__fat_g', 'food_name__sugar_g'
#         )
#         print(user_meals, "usermeals@@@@")
#         meal_nutrient = []
...
#         print("Meal Nutrient: ", meal_nutrient)
#         carbs = self.sum_nutrients(meal_nutrient, 'carbs')
#         prot = self.sum_nutrients(meal_nutrient, 'protein')
#         fat = self.sum_nutrients(meal_nutrient, 'fat')
#         sugar = self.sum_nutrients(meal_nutrient, 'sugar')

#         return carbs, prot, fat, sugar

#     def sum_nutrients(self, meal_nutrient, nutrient_key):
#         return sum(item[nutrient_key] for item in meal_nutrient)

#     def evaluate_user_meal(self, token, meal_time):
#         user_info = self.get_user_info(token)
        
#         user_id = user_info.get('uuid', '')
#         user_height = user_info.get('height', '')
#         user_weight = user_info.get('weight', '')
#         user_active_level = user_info.get('active_level', '')
#         user_birthdate = user_info.get('birthdate', '')
#         user_diet_purpose = user_info.get('diet_purpose', '')
#         user_gender = user_info.get('gender', '')
        
#         user_data = (user_height, user_weight, user_birthdate, user_gender, user_active_level, user_diet_purpose)
#         recommend_nutrients = self.calculate(*user_data)
        
#         user_meal_nut = self.get_user_meal(user_id, meal_time)
#         diet_rating = self.evaluate(user_meal_nut, recommend_nutrients)
...
#     def calculate_protein(self, weight, activity_level):
#         protein_factors = {
#             1: (0.8, 0.9),
#             2: (1.0, 1.2),
#             3: (1.2, 1.5),
...
#     def calculate(self, height, weight, birth_date, sex, activity_level, goal):
...
#         protein = self.calculate_protein(weight, activity_level)
#         fat = tdee * 0.2 / 9
#         carbs = ((tdee - (protein[0] + fat * 9)) / 4, (tdee - (protein[1] + fat * 9)) / 4)
#         sugar = tdee * 0.1 / 4
...
#     def evaluate(self, user_meal_nut, recommend):
#         def calculate_error(recommend, actual):
#             if isinstance(recommend, tuple):
#                 min_error = abs((actual - recommend[0])) / recommend[0] * 100
#                 max_error = abs((actual - recommend[1])) / recommend[1] * 100
...
#         carbs_error = calculate_error(recommend['carbs'], user_meal_nut['carbs'])
#         protein_error = calculate_error(recommend['protein'], user_meal_nut['protein'])
#         fat_error = calculate_error(recommend['fat'], user_meal_nut['fat'])
...
#     def save_user_evaluation(self, user_uid, meal_time, diet_rating, user_meal_nut):
#         new_user_evaluation_data = Usermealevaluation(
#             user_id=user_uid, meal_date=meal_time, sum_carb=user_meal_nut[0], sum_protein=user_meal_nut[1],
#             sun_fat=user_meal_nut[2], meal_evaluation=diet_rating
#         )
